...
def stereo_rectification(imgL, imgR, K1, K2, R, T):
    ...
    map1x, map1y = cv2.initUndistortRectifyMap(
        K1, dist1, R1, P1, (w, h), cv2.CV_32FC1
    )

    map2x, map2y = cv2.initUndistortRectifyMap(
        K2, dist2, R2, P2, (w, h), cv2.CV_32FC1
    )
...
def compute_disparity(rectL, rectR):
    ...
    return disparity, disp_vis

# =====================================================
# PROFONDEUR
# =====================================================

def draw_rectified_lines(imgL, imgR):
    ...
    for y in range(0, h, 40):
        ...
        cv2.line(imgL, (0, y), (imgL.shape[1], y), (0,255,0), 1)
        cv2.line(imgR, (0, y), (imgR.shape[1], y), (0,255,0), 1)

    return imgL, imgR

def test_fundamental_matrix(ptsL, ptsR, F):
    ...
    for pL, pR in zip(ptsL, ptsR):
        ...
        xL = np.array([pL[0], pL[1], 1])
        xR = np.array([pR[0], pR[1], 1])

        err = abs(xR.T @ F @ xL)

        errors.append(err)

    print("Erreur moyenne contrainte épipolaire :", np.mean(errors))
...
rectL, rectR, Q = stereo_rectification(imgL, imgR, K1, K2, R, T)

cv2.imshow("Rectified Left", rectL)
cv2.imshow("Rectified Right", rectR)

rectL_lines, rectR_lines = draw_rectified_lines(rectL, rectR)

cv2.imshow("Rectified Left Lines", rectL_lines)
cv2.imshow("Rectified Right Lines", rectR_lines)

disparity, disp_vis = compute_disparity(rectL, rectR)
